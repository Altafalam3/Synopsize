...
if text_length <= 1000:
    max_length = int(0.3 * text_length)
elif text_length <= 15000:
    max_length = int(0.20 * text_length)
elif text_length >= 50000:
    max_length = int(0.10 * text_length)
else:
    max_length = int(0.15 * text_length)

# Tokenize input text and generate summary
input_ids = tokenizer.encode(transcribed_text, return_tensors="pt")
summary_ids = model.generate(input_ids, num_beams=4, length_penalty=2.0, max_length=max_length, min_length=56, no_repeat_ngram_size=3, early_stopping=True)
